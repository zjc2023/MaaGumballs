...
class PoolTrick_Test(CustomAction):
    def getArmorATK(self, context: Context)-> int:
        ...
        armorATK = context.run_task("OCRArmorATK").nodes[0].recognition.best_result.text
    ...
    def run(
        ...
        #先找到泉水位置
        searchpoolpos = None
        while searchpoolpos is None:
            searchpoolpos = context.run_task("SearchPool").nodes[0].recognition.best_result
        searchpoolpos = searchpoolpos.box
