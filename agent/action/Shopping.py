...
class Shopping(CustomAction):
    ...
    def CheckShopListWindows(self, context: Context):
        recoDetail = context.run_recognition("CheckShopListWindows",
    ...
    def EnterShop(self, context: Context, ShopName: str): 
        ...
        recoDetail = context.run_recognition("EnterShop",
                                img,
                                pipeline_override={
                                        "EnterShop": {
                                        "recognition": "OCR",
    ...
    def Shop_ShoppingRewards_Check(self, context: Context):
        # 检查是否触发商人奖励
        img = context.tasker.controller.post_screencap().wait().get()
        recoDetail = context.run_recognition("Shop_ShoppingRewards_Check",
                                img,
                                pipeline_override={
    ...
    def Shop_GoldCoins(self, context: Context):
        ...
        while SwipeCount > 0:
            img = context.tasker.controller.post_screencap().wait().get()
            recoDetail = context.run_recognition("Shop_FindGoldCions_reco",
                        img,
                        pipeline_override={
    ...
    def Shop_RuinCoins(self, context: Context):
        ...
        while SwipeCount > 0:
            img = context.tasker.controller.post_screencap().wait().get()
            recoDetail = context.run_recognition("Shop_RuinCoins_reco",
                        img,
                        pipeline_override={
                            "Shop_RuinCoins_reco": {
                                "recognition": "TemplateMatch",
            ...
            if recoDetail:
                ...
                for result in recoDetail.all_results:
                    box = result.box
                    context.tasker.controller.post_click(
                        int(box[0] + box[2] / 2) + 40,
                        int(box[1] + box[3] / 2) - 80
                    )
                    time.sleep(0.5)
                    context.run_task("ConfirmButton")
        ...
        # 检查是否存在符石商店
        img = context.tasker.controller.post_screencap().wait().get()
        recoDetail = context.run_recognition("Shop_Runestone_reco",
                        img,
                        pipeline_override={
                            "Shop_Runestone_reco": {
                                "recognition": "OCR",
    ...
    def Shop_Mercenary(self, context: Context):
        # 检查是否存在佣兵商店
        img = context.tasker.controller.post_screencap().wait().get()
        recoDetail = context.run_recognition("Shop_Mercenary_reco",
                        img,
                        pipeline_override={
                            "Shop_Mercenary_reco": {
                                "recognition": "OCR",
